...
// Unset_vma cancels the current page of 
void unset_vma(struct task_struct *task, uint64_t addr, uint64_t length, uint64_t flags,
...
void unset_created(uint64 *pgtbl, uint64 va){
	create_mapping(pgtbl, va, 0, 1, 0, 1, 1, 0);
}
...
uint64 copy_pgdir(struct task_struct * father, struct task_struct * new){
	...
	create_mapping(new_pgdir, USER_END - PGSIZE, stack - PA2VA_OFFSET, 1, VM_WRITE | VM_READ | VM_USER | VM_VALID, 1, 0, 0);
	...
	for (struct vm_area_struct * vma = father->mm.mmap; vma != NULL; vma = vma->vm_next){
		for (uint64_t pg_start = PGROUNDDOWN(vma->vm_start); pg_start < vma->vm_end; pg_start += PGSIZE){
			...
			if (check != 0){
				...
				uint64_t flag = (vma->flags << 1) | VM_USER | VM_VALID;
