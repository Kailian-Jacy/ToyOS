...
struct task_struct* get_task(int i){
	return task[i];
}

// Load ELF. Loads uapp only.
void load_origin(int i){
	// Now ELF is linked into UAPP_START.
	Elf64_Ehdr * header = (Elf64_Ehdr *) fork_start;
	...
	for (int j = 0; j< EI_NIDENT; j++){
		if (header_magic[j] != header->e_ident[j]) {
			printk("load_origin.header.magic\n");
		}
	}
	...
	for (int j = 0; j < phdr_cnt; j++)
	{
		...
		if (phdr->p_type == PT_LOAD)
		{
			...
			set_vma(task[i], phdr->p_vaddr, phdr->p_memsz, phdr->p_flags, 
					phdr->p_offset, phdr->p_filesz);
		}
	}
	// allocate user stack and do mapping
	set_vma(task[i],USER_END - PGSIZE, PGSIZE, VM_READ | VM_WRITE, 0, 0);
	uint64_t stack = alloc_page();
	create_mapping(task[i]->pgd + PA2VA_OFFSET, USER_END - PGSIZE, stack - PA2VA_OFFSET, 1, VM_WRITE | VM_READ | VM_USER | VM_VALID, 1, 0, 0);
...
void dummy() {
	...
	// printk("[PID = %d] This is Dummy! variable: %d\n", current->pid, global_variable++);
	// int pid = sys_fork();
	// if (pid == 0){
		// Child.
		// exec();
		while (1){
			printk("[PID = %d] If I appears, this is a child process. variable: %d\n", current->pid, global_variable++);
    ...
    //         printk("[PID = %d] Dummy is running. auto_inc_local_var = %d\n", current->pid, auto_inc_local_var);
    //     }
    // }
}
...
void do_page_fault(struct pt_regs *regs){
	...
	uint64_t flag = (vma->flags << 1) | VM_VALID;
	...
	if (pg_start >= vma->vm_start){
		...
		file_start = (uint64_t)current->mm.file + (vma->file_offset + pg_start - vma->vm_start);
	} else {
		...
		file_start = (uint64_t)current->mm.file + vma->file_offset;
	}
